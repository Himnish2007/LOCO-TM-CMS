-- ================================================================
-- LOCO TM CMS - RUT200 Script v13.0 FINAL
-- Himnish Limited
-- Fix: isValid check - only process data when sensor is connected
-- ================================================================
...
-- Parse BCM0003 with isValid check
local function parse_sensor(json_chunk)
  -- KEY FIX: Check isValid first
  -- If sensor not connected, BNI sends isValid:false
  ...
  local ok1,vx = pcall(f32,b[1],b[2],b[3],b[4]); if not ok1 then return nil,"f32 err" end
...
-- Process buffer — extract JSON chunks and check isValid
local function process_buffer(buf, tm_data)
  local pos=1
  while true do
    -- Find complete JSON object starting with {"timestamp"
    ...
    -- Find which port this belongs to (look back 200 bytes)
    ...
    for port,tm in pairs(PORT_MAP) do
      if lookback:find(port,1,true) then
        ...
        if sensor then
          tm_data[tm]=sensor
          io.write(tm.."("..sensor.temp.."C "..sensor.vib.rms..") ")
          io.flush()
        else
          -- Sensor disconnected — send DISCONNECTED status
          if reason=="disconnected" then
            tm_data[tm]={
              vib={x=0,y=0,z=0,rms=0,peak=0,crestFactor=0,freq=0},
              temp=0,
              ioLinkStatus="DISCONNECTED"
            }
...
-- MAIN
print("LOCO TM CMS v13.0 | Himnish Limited")
print("Server: "..RAILWAY)
print("isValid check: ENABLED")
local tm_data={}
...
while true do
  ...
  if not c then
    ...
    socket.sleep(0.5)
    print("Subscribed all 6 ports! Listening...")
    c:settimeout(2)
    buf_acc=""
